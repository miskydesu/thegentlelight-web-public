...
import { adminCreateQuoteWithGpt, adminListQuoteTags, adminCheckDuplicateQuote, clearAdminToken } from '../../../../lib/tglAdminApi'

export default function AdminQuoteNewGptPage() {
  ...
  const handleGenerateAndCreate = async (e: React.FormEvent) => {
    ...
    try {
      // 重複チェック（日本語のquote_textのみ）
      const duplicateCheck = await adminCheckDuplicateQuote(quoteTextJa.trim() || undefined, undefined)
      ...
      const res = await adminCreateQuoteWithGpt({
        ...
        author_name: authorNameJa || null,
      })

      const quoteId = res.quote.quote_id

      // 編集ページにリダイレクト
      router.push(`/admin/quotes/${quoteId}?lang=ja`)
    } catch (err: any) {
      ...
      <form onSubmit={handleGenerateAndCreate}>
        ...
        >
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 20 }}>
            ...
            >
              <span style={{ color: '#495057', fontSize: '0.85rem', fontWeight: 500 }}>
                名言本文（日本語） <span style={{ color: '#dc3545' }}>*</span>
              </span>
              <textarea
                value={quoteTextJa}
                onChange={(e) => setQuoteTextJa(e.target.value)}
                required
                rows={6}
                placeholder="名言本文を入力してください（日本語）"
                style={{
                  padding: '10px 12px',
                  border: '1px solid #ced4da',
                  borderRadius: '4px',
            ...
            >
              <span style={{ color: '#495057', fontSize: '0.85rem', fontWeight: 500 }}>著者名（日本語）</span>
              <input
                value={authorNameJa}
                onChange={(e) => setAuthorNameJa(e.target.value)}
                placeholder="著者名を入力してください（日本語）"
                style={{
                  padding: '10px 12px',
                  border: '1px solid #ced4da',
                  borderRadius: '4px',
            ...
            >
              <span style={{ color: '#495057', fontSize: '0.85rem', fontWeight: 500 }}>出典（日本語）</span>
              <input
                value={sourceTextJa}
                onChange={(e) => setSourceTextJa(e.target.value)}
                placeholder="出典情報を入力してください（日本語）"
                style={{
                  padding: '10px 12px',
                  border: '1px solid #ced4da',
                  borderRadius: '4px',
