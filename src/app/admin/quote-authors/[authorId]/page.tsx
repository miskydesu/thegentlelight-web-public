...
import { adminGetQuoteAuthor, adminUpdateQuoteAuthor, adminUploadImage, adminUploadTempImage, clearAdminToken, type AdminQuoteAuthor } from '../../../../lib/tglAdminApi'
...
export default function AdminQuoteAuthorEditPage() {
  ...
  const [row, setRow] = useState<AdminQuoteAuthor | null>(null)
  ...
  const load = async () => {
    ...
    setBusy(true)
    try {
      const res = await adminGetQuoteAuthor(authorId)
      const author = res.author
      setRow(author)
      setCanonicalKey(author.canonical_key || '')
      setIsPublished(author.is_published ?? true)
      setDisplayNameJa(author.localizations?.ja?.display_name || '')
      setDisplayNameEn(author.localizations?.en?.display_name || '')
      setOneLinerJa(author.localizations?.ja?.one_liner || '')
      setOneLinerEn(author.localizations?.en?.one_liner || '')
      setDetailJa(author.localizations?.ja?.detail_md || '')
      setDetailEn(author.localizations?.en?.detail_md || '')
      setSeoTitleJa(author.localizations?.ja?.seo_title || '')
      setSeoTitleEn(author.localizations?.en?.seo_title || '')
      setSeoDescJa(author.localizations?.ja?.seo_description || '')
      setSeoDescEn(author.localizations?.en?.seo_description || '')
      const merged = (author.aliases || []).join('\n')
      setAliasesTextJa(merged)
      setAliasesTextEn(merged)
      setType(author.type || '')
      setImageKey(author.image_key || '')
      setImageUrl(author.image_key ? buildImageUrl(author.image_key) : null)
      setLinksJsonJa(author.localizations?.ja?.links ? JSON.stringify(author.localizations.ja.links, null, 2) : '')
      setLinksJsonEn(author.localizations?.en?.links ? JSON.stringify(author.localizations.en.links, null, 2) : '')
    } catch (err: any) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      await adminUpdateQuoteAuthor(authorId, {
  ...
  return (
    <main style={{ maxWidth: 1200, margin: '0 auto', padding: 24 }}>
      ...
      <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
        ...
        {row?.author_id ? <span style={{ color: '#6c757d', fontSize: '0.85rem' }}>{row.author_id}</span> : null}
      </div>
    </main>
  )
}
