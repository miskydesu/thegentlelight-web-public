...
export function generateMetadata({ searchParams }: { searchParams: { q?: string; theme?: string } }) {
...
export default async function EnQuotesPage({ searchParams }: { searchParams: { q?: string; theme?: string } }) {
  ...
  const apiPath = `/v1/${sourceCountry}/quotes?limit=30${q ? `&q=${encodeURIComponent(q)}` : ''}`
